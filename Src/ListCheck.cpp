...
{
	ListView_SetExtendedListViewStyle(m_ctrList.m_hWnd, LVS_EX_FULLROWSELECT|LVS_EX_GRIDLINES);
	...
	m_ctrList.InsertColumn(0, _T("Contents"), LVCFMT_LEFT, 120);
	m_ctrList.InsertColumn(1, _T("status"), LVCFMT_LEFT, 70);
	m_ctrList.InsertColumn(2, _T("site"), LVCFMT_LEFT, 250);
	m_ctrList.InsertColumn(3, _T("IP"), LVCFMT_LEFT, 110);
//	m_ctrList.InsertColumn(4, _T("Comment"), LVCFMT_LEFT, 110);
...
//
// ================================================================================
void CListCheck::SelectRow(int nRow)
{

	ListView_SetItemState(m_ctrList.m_hWnd,-1,0,LVIS_SELECTED | LVIS_FOCUSED);
	m_ctrList.EnsureVisible(nRow, FALSE); 
	m_ctrList.SetSelectionMark(nRow); 
	m_ctrList.SetItemState(nRow, LVIS_SELECTED | LVIS_FOCUSED, LVIS_SELECTED | LVIS_FOCUSED); 
}
...
void CListCheck::OnGetdispinfoList(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	if (pItem->mask & LVIF_TEXT)
	{
		...
		{
			text = m_database[itemid].m_Status;
			//Text is slogan
		}
